...
class GIN_delta(nn.Module):
    ...
    def __init__(self, in_feats, n_hidden, n_classes, n_layers, dropout, feat_init_num):
        ...
        if n_layers > 1:
            self.layers.append(GINConv(Linear(in_feats, n_hidden)))
            for i in range(1, n_layers - 1):
                self.layers.append(GINConv(Linear(n_hidden, n_hidden)))
            self.layers.append(GINConv(Linear(n_hidden, n_classes)))
        else:
            self.layers.append(GINConv(Linear(in_feats, n_classes)))
