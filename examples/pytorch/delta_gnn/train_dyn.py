...
def main(args):
    ...
    print("--------------------------")
    print('Initial graph')
    checkpt_file_wo_retrain = 'pretrained/wo_retrain/' + uuid.uuid4(
    ).hex + '.pt'
    ...
    # create GNN model
    if model_name == 'gcn':
        model_wo_retrain = GCN(in_feats, n_hidden, n_classes, n_layers,
                               dropout).to(device)
        model_retrain = GCN(in_feats, n_hidden, n_classes, n_layers,
                            dropout).to(device)
        model_delta = GCN_delta(in_feats, n_hidden, n_classes, n_layers,
                                dropout, features.shape[0]).to(device)
    elif model_name == 'graphsage':
        model_wo_retrain = GraphSAGE(in_feats, n_hidden, n_classes, n_layers,
                                     dropout).to(device)
        model_retrain = GraphSAGE(in_feats, n_hidden, n_classes, n_layers,
                                  dropout).to(device)
        model_delta = GraphSAGE_delta(in_feats, n_hidden, n_classes, n_layers,
                                      dropout, features.shape[0]).to(device)
    elif model_name == 'gat':
        model_wo_retrain = GAT(in_feats, n_hidden, n_classes, n_layers,
                               feat_dropout, heads).to(device)
        model_retrain = GAT(in_feats, n_hidden, n_classes, n_layers,
                            feat_dropout, heads).to(device)
        model_delta = GAT_delta(in_feats, n_hidden, n_classes, n_layers,
                                feat_dropout, heads,
                                features.shape[0]).to(device)
    elif model_name == 'gin':
        model_wo_retrain = GIN(in_feats, n_hidden, n_classes, n_layers,
                               dropout).to(device)
        model_retrain = GIN(in_feats, n_hidden, n_classes, n_layers,
                            dropout).to(device)
        model_delta = GIN_delta(in_feats, n_hidden, n_classes, n_layers,
                                dropout, features.shape[0]).to(device)
    elif model_name == 'deepergcn':
        model_wo_retrain = DeeperGCN(in_feats, n_hidden, n_classes, n_layers,
                                     dropout).to(device)
        model_retrain = DeeperGCN(in_feats, n_hidden, n_classes, n_layers,
                                  dropout).to(device)
        model_delta = DeeperGCN_delta(in_feats, n_hidden, n_classes, n_layers,
                                      dropout, features.shape[0]).to(device)
    ...
    train_loader, valid_loader, test_loader = gen_dataloader(
        edge_index_evo, train_idx, val_idx, test_idx, features, labels,
        num_nghs, batch_size)
    # Gen edge_dict
    ...
    acc_wo_retrain = test(model_wo_retrain, test_loader, device,
                          checkpt_file_wo_retrain)

    # Retrain
    print('### Model_retrain:')
    # edge_index_retrain = edge_idx_init.clone().detach()
    t0 = time.time()
    train(args, model_retrain, train_loader, valid_loader, device,
          checkpt_file_retrain, lr, weight_decay)
    acc_retrain = test(model_retrain, test_loader, device,
                       checkpt_file_retrain)
    ...
    t0 = time.time()
    train_delta(args, model_delta, train_loader, valid_loader, device,
                checkpt_file_delta, lr, weight_decay)
    acc_delta = test_delta(model_delta, test_loader, device,
                           checkpt_file_delta)
    delta_time += time.time() - t0
    # acc_delta = 0

    accuracy.append(
        [0, acc_wo_retrain * 100, acc_retrain * 100, acc_delta * 100])
    ...
    print('------------------ update -------------------')
    snapList = [
        f for f in glob('./data/' + args.dataset + '/*Edgeupdate_snap*.txt')
    ]
    ...
    for i in range(len(snapList)):
        ...
        edge_dict, edge_index_evo = insert_edges_evo(edge_index_evo, edge_dict,
                                                     inserted_edge_index, 0,
                                                     n_layers)
        train_loader, valid_loader, test_loader = gen_dataloader(
            edge_index_evo, train_idx, val_idx, test_idx, features, labels,
            num_nghs, batch_size)
        ...
        edge_dict_delta, edge_index_evo_delta, v_sen, v_insen, comp_total, comp_delta, access_total, access_delta = insert_edges_delta(
            edge_index_evo_delta, edge_dict_delta, inserted_edge_index,
            threshold, n_layers)

        print('Node_num: ', len(v_sen), len(v_insen))
        sensitive_ratio.append(
            round(len(v_sen) * 100 / (len(v_sen) + len(v_insen)), 2))
        ...
        comp_reduction = round(
            (comp_total_accumulate - comp_delta_accumulate) /
            comp_total_accumulate * 100, 2)
        access_reduction = round(
            (access_total_accumulate - access_delta_accumulate) /
            access_total_accumulate * 100, 2)
        access_reduct_list.append(access_reduction)
        comp_reduct_list.append(comp_reduction)
        print('Access reduction: {:d}/{:d} = {:.2f}%'.format(
            access_delta, access_total, access_reduction))
        print('Computation reduction: {:d}/{:d} = {:.2f}%'.format(
            comp_delta, comp_total, comp_reduction))

        train_loader_delta, valid_loader_delta, test_loader_delta = gen_dataloader(
            edge_index_evo_delta, train_idx, val_idx, test_idx, features,
        ...
        t0 = time.time()
        train_delta(args, model_delta, train_loader_delta, valid_loader_delta,
                    device, checkpt_file_delta, lr, weight_decay, v_sen,
                    v_insen)
        acc_delta = test_delta(model_delta, test_loader_delta, device,
                               checkpt_file_delta, v_sen, v_insen)
        delta_time += time.time() - t0

        accuracy.append(
            [i + 1, acc_wo_retrain * 100, acc_retrain * 100, acc_delta * 100])

    acc_degrad = []
    for i in range(len(accuracy)):
        if i == 0:
            print('{:d}\t{:.2f}  {:.2f}  {:.2f}'.format(
                i, accuracy[i][1], accuracy[i][2], accuracy[i][3]))
        else:
            ...
            print(
                '{:d} {:.2f}  {:.2f}  {:.2f} {:.2f} mem: {:.2f}%  comp: {:.2f}%  sen:{:.2f}%'
                .format(i, accuracy[i][1], accuracy[i][2], accuracy[i][3],
                        delta_acc, access_reduct_list[i - 1],
    ...
    print('Time cost of retrain: [{}] and delta: [{}]'.format(
        util.time_format(retrain_time), util.time_format(delta_time)))
    ...
    acc_dump = [
        acc_dump[i] + [sensitive_ratio[i]] for i in range(len(acc_dump))
    ]
    np.savetxt('./results/' + args.dataset + '_' + args.model + '_th' +
               str(args.threshold) + '_acc.txt',
               acc_dump,
               fmt='%.2f, %.2f, %.2f, %.2f')
    ...
    np.savetxt('./results/' + args.dataset + '_' + args.model + '_th' +
               str(args.threshold) + '_reduction.txt',
               reduction,
               fmt='%.2f, %.2f')


def gen_dataloader(edge_index, train_idx, val_idx, test_idx, features, labels,
                   num_nghs, batch_size):
    features = torch.FloatTensor(features)
    data = Data(x=features, edge_index=edge_index, y=labels)
    data.n_id = torch.arange(
        data.num_nodes)  # Keep the original vertex ID of global
    del features
    gc.collect()
...
def train(args, model, train_loader, valid_loader, device, checkpt_file, lr,
          weight_decay):
    bad_counter = 0
    best = 0
    best_epoch = 0
    ...
    for epoch in range(args.epochs):
        if args.model == 'gcn':
            loss_tra, train_ep = gcn.train(model, device, train_loader, lr,
                                           weight_decay)
        elif args.model == 'graphsage':
            loss_tra, train_ep = graphsage.train(model, device, train_loader,
                                                 lr, weight_decay)
        elif args.model == 'gat':
            loss_tra, train_ep = gat.train(model, device, train_loader, lr,
                                           weight_decay)
        elif args.model == 'gin':
            loss_tra, train_ep = gin.train(model, device, train_loader, lr,
                                           weight_decay)
        elif args.model == 'deepergcn':
            loss_tra, train_ep = deepergcn.train(model, device, train_loader,
                                                 lr)
        ...
        if (epoch + 1) % 20 == 0:
            print(
                'Epoch:{:02d}, Train_loss:{:.3f}, Valid_acc:{:.2f}%, Time_cost:{:.3f}/{:.3f}'
                .format(epoch + 1, loss_tra, 100 * f1_val, train_ep,
                        train_time))
            # print('Remove print')
        if f1_val > best:
            best = f1_val
...
def train_delta(args,
    ...
    for epoch in range(args.epochs):
        if args.model == 'gcn':
            loss_tra, train_ep = gcn.train_delta(model, device, train_loader,
                                                 lr, weight_decay,
                                                 v_sen_feat_loc_train,
                                                 v_insen_feat_loc_train,
                                                 v_sen_train, v_insen_train)
        elif args.model == 'graphsage':
            # Remapping feat_loc during each iteration of training, due to the random sampling in graphSAGE
            loss_tra, train_ep = graphsage.train_delta(model, device,
                                                       train_loader, lr,
                                                       weight_decay, v_sen,
                                                       v_insen)
        elif args.model == 'gat':
            loss_tra, train_ep = gat.train_delta(model, device, train_loader,
                                                 lr, weight_decay,
                                                 v_sen_feat_loc_train,
                                                 v_insen_feat_loc_train,
                                                 v_sen_train, v_insen_train)
        elif args.model == 'gin':
            loss_tra, train_ep = gin.train_delta(model, device, train_loader,
                                                 lr, weight_decay,
                                                 v_sen_feat_loc_train,
                                                 v_insen_feat_loc_train,
                                                 v_sen_train, v_insen_train)
        elif args.model == 'deepergcn':
            loss_tra, train_ep = deepergcn.train_delta(
                model, device, train_loader, lr, v_sen_feat_loc_train,
                v_insen_feat_loc_train, v_sen_train, v_insen_train)

        t_st = time.time()

        if args.model == 'gcn':
            f1_val = gcn.validate_delta(model, device, valid_loader,
                                        v_sen_feat_loc_val,
                                        v_insen_feat_loc_val, v_sen_val,
                                        v_insen_val)
        elif args.model == 'graphsage':
            # Remapping feat_loc during each iteration of training, due to the random sampling in graphSAGE
            f1_val = graphsage.validate_delta(model, device, valid_loader,
                                              v_sen, v_insen)
        elif args.model == 'gat':
            f1_val = gat.validate_delta(model, device, valid_loader,
                                        v_sen_feat_loc_val,
                                        v_insen_feat_loc_val, v_sen_val,
                                        v_insen_val)
        elif args.model == 'gin':
            f1_val = gin.validate_delta(model, device, valid_loader,
                                        v_sen_feat_loc_val,
                                        v_insen_feat_loc_val, v_sen_val,
                                        v_insen_val)
        elif args.model == 'deepergcn':
            f1_val = deepergcn.validate_delta(model, device, valid_loader,
                                              v_sen_feat_loc_val,
                                              v_insen_feat_loc_val, v_sen_val,
                                              v_insen_val)
...
def test_delta(model,
               ...
               v_insen=None):
    if args.model == 'gcn':
        test_acc = gcn.test_delta(model, device, test_loader, checkpt_file,
                                  v_sen, v_insen)
    elif args.model == 'graphsage':
        test_acc = graphsage.test_delta(model, device, test_loader,
                                        checkpt_file, v_sen, v_insen)
    elif args.model == 'gat':
        test_acc = gat.test_delta(model, device, test_loader, checkpt_file,
                                  v_sen, v_insen)
    elif args.model == 'gin':
        test_acc = gin.test_delta(model, device, test_loader, checkpt_file,
                                  v_sen, v_insen)
    elif args.model == 'deepergcn':
        test_acc = deepergcn.test_delta(model, device, test_loader,
                                        checkpt_file, v_sen, v_insen)
    print('Test accuracy:{:.2f}%'.format(100 * test_acc))
    return test_acc
...
if __name__ == '__main__':
    ...
    log_name = args.model + '_' + args.dataset + '_' + time.strftime(
        "%m%d_%H%M", time.localtime()) + '.txt'
    log_file = './results/log/' + log_name
    # print(log_file)
