...
def main(args):
    ...
    features, labels, train_idx, val_idx, test_idx, n_classes, edge_index_init = load_dataset_full(
        args.dataset)  ##
    ...
    for i in range(len(snapList)):
        ...
        # edge_index_deleted = load_updated_edges(args.dataset, i + 1)
        delete_ratio = 0.1
        edge_index_deleted = delete_edges_random(edge_index_evo, delete_ratio)

        # edge_index_evo = insert_edges(edge_index_evo, inserted_edge_index)
        # edge_dict_tmp = cp.deepcopy(edge_dict)
        edge_dict_deleted, edge_index_evo = delete_edges_evo(cp.deepcopy(edge_dict), edge_index_deleted, 0, n_layers)
        train_loader, valid_loader, test_loader = gen_dataloader(edge_index_evo, train_idx, val_idx,
        ...
        edge_dict_deleted, edge_index_evo_delta, v_sen, v_insen, comp_total, comp_delta, access_total, access_delta = delete_edges_delta(
            cp.deepcopy(edge_dict), edge_index_deleted, threshold, n_layers)
