    ...
    def forward(self,
        ...
        * Weight shape: :math:`(\text{in_feats}, \text{out_feats})`.
        ...
        with graph.local_scope():
            ...
            # # Delta aggregate according to node degree
            # # feat_tmp = self.feat_prev.clone()
            # feat_aggr = th.zeros([feat.size()[0], feat.size()[1]])
            # # Nodes with high degree
            # for root_node, ngh_nodes in nodes_high_deg.items():
            ...
            # for root_node, ngh_nodes in nodes_low_deg.items():
            #     node_feat = th.Tensor(feat.size()[1])
            #     for node in ngh_nodes:
            #         node_feat = node_feat + feat_src[node]
            #     if weight is not None:
            #         node_feat = th.matmul(node_feat, weight)
            #     feat_aggr[root_node] = feat_aggr[root_node] + node_feat + self.feat_prev[node]

            # rst = feat_aggr
            # self.feat_prev = feat_aggr

            # Delta aggregates
            # feat_src = feat_src.to(device)
            feat_src = feat_src
            feat_aggr = th.zeros([feat.size()[0], feat.size()[1]])
            ...
            for root_node, ngh_nodes in ngh_dict.items():
                # node_feat = th.Tensor(feat.size()[1]).to(device)
                node_feat = th.Tensor(feat.size()[1])
                for node in ngh_nodes:
                    node_feat += feat_src[node]
                feat_aggr[root_node] = node_feat
